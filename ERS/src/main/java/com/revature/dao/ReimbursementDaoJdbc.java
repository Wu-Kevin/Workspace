...
public class ReimbursementDaoJdbc implements ReimbursementDao {
	...
	public Reimbursement insert(Reimbursement reimbursement) {
		try (Connection conn = ConnectionUtil.getConnection()) {
			...
			sql = "SELECT * FROM REIMBURSEMENT WHERE EMPLOYEE_ID = ?" + "AND REQUEST_AMOUNT = ?" + "AND RATIONALE = ?";
			...
			while (rs.next()) {
				return new Reimbursement(rs.getInt("REIMBURSEMENT_ID"), rs.getInt("EMPLOYEE_ID"),
						rs.getDouble("REQUEST_AMOUNT"), rs.getString("RATIONALE"), rs.getInt("APPROVAL"),
	...
	@Override
	public void update(int reimbursementid, int managerid, int approval) {
		try (Connection conn = ConnectionUtil.getConnection()) {
			String sql = "UPDATE REIMBURSEMENT SET APPROVAL_ID = ?, APPROVAL = ?"
			...
			LogUtil.logger.warn("Exception updating reimbursements", e);
		}
	}
	...
	public List<Reimbursement> selectByID(int employeeid) {
		try (Connection conn = ConnectionUtil.getConnection()) {
			...
			while (rs.next()) {
				reimbursementList.add(new Reimbursement(rs.getInt("REIMBURSEMENT_ID"), rs.getInt("EMPLOYEE_ID"),
						rs.getDouble("REQUEST_AMOUNT"), rs.getString("RATIONALE"), rs.getInt("APPROVAL"),
						rs.getInt("APPROVAL_ID")));
			}
	...
	public List<Reimbursement> selectAll() {
		try (Connection conn = ConnectionUtil.getConnection()) {
			...
			while (rs.next()) {
				reimbursementList.add(new Reimbursement(rs.getInt("REIMBURSEMENT_ID"), rs.getInt("EMPLOYEE_ID"),
						rs.getDouble("REIMBURSEMENT_ID"), rs.getString("RATIONALE"), rs.getInt("APPROVAL"),
						rs.getInt("APPROVAL_ID")));
			}
